...
**未解決の留保(Low)**: テラコッタの値は**トピック文字からの採取**であり、**塗りつぶしバッジ本体**を
測定したものではない。バッジが実際に塗りつぶし要素として存在する場合、本体色とテキスト色が
異なる可能性が残る。#1玄関の絵コンテ確認時に、バッジの見た目が意図通りか目視で確認すること
(11章チェックリストに追加)。
...
- [ ] トピックバッジの見た目が意図通りか(塗りつぶしかテキストのみか。9-0章の留保参照)
...
| 6 | Higgsfieldでのレンダリング費用 | **状況証拠**(残高不変・取引履歴に該当項目なし)は得たが未確定。企画書A-4参照 |
