...
### 解答円と背景要素の重なりに関する評価基準(2026-09-13確定)

`【事実】` #1・#5・#10では「解答円が対象物の**支持面**(床・棚板・天板等、対象物が置かれている面そのもの)
に重なる」ケースをLow評価としてきた。**#6・#7・#9の納品で、支持面だけでなく独立したダミー要素
(ラグの輪郭・椅子の背もたれ)の一部に円が重なるケースが新たに発生**したため、判定基準を明文化する。

| 重なりの種類 | 評価 | 理由 |
|---|---|---|
| 対象物自身の支持面(床・棚板・天板・枠) | **Low**(既存基準どおり) | 「置かれている」ことを示す構図上、接触点を含むのは自然かつ必然 |
| 対象物と同一の家具グループに属す別要素(例: テレビ台とテレビ、本棚と本) | **Low** | 一枚絵の要素リスト上、同一ダミーとして扱われており、独立した別NGとの混同が起きない |
| **独立した別のダミー装飾(ラグ・椅子等)の輪郭線に部分的に重なる**(#6 NG②とラグ、#7 NG②と椅子、#9 NG①とラグ) | **Low(今回、条件付きで承認)** | 円は輪郭の一部と交差するのみで対象物を隠さず、他のNG・独立小物を完全に包含しない。ただし**装飾ダミー自体を誤って対象と誤認させる規模の重なり(輪郭の大部分を囲む等)は不可** |
| NGマーカー同士の重なり・対象物の遮蔽 | **不可** | 4章1項(3箇所を離す)に違反 |

**運用**: 上表の3行目に該当する新規パターンは、今後Codexからの逸脱報告で個別に位置(中心座標・重なり対象)を
明記させ、Claudeが上表の基準で都度判定する。基準に当てはまらない重なり方(対象物の遮蔽、輪郭の大部分包含等)
は差し戻す。
...
**Metricool予約投稿**: 実行済み。投稿ID`375106106`、2026-09-26 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。動画URLは2026-09-13にコーデックスへ再送依頼し受領。
...
**Metricool予約投稿**: 実行済み。投稿ID`375107608`、2026-09-27 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。動画URLは2026-09-13にコーデックスへ再送依頼し受領。
...
### #6 リビング 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式(HANDOFF全文・ffprobe生出力・絵コンテ・音声詳細・
残高記録)を受領。SHA-256を含む同梱ファイルの整合性をClaudeが独立に検証し、改変・破損なしを確認。
絵コンテ画像を目視確認済み。プロジェクトID `living06`。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 13,150,127バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| 設計書11章チェックリスト | 17項目中、9項目に技術的な逸脱の言及あり(下記参照)。他は合格 |
| 絵コンテ | 目視確認済み(9コマ、Claude確認OK) |
| **逸脱1: 解答円と背景の重なり** | NG①②③(a)(c)は対象物自身の支持面(ソファ・テーブル・本棚)への接触でLow(既存基準)。**NG②はテーブル前縁に加えラグの輪郭にも一部重なる**→4章新基準の3行目に該当。円がラグを覆う面積は小さく対象物(書類)を隠していないため**Low(条件付き承認)** |
| 逸脱2: 円描画の秒数 | 0.25秒指定を8フレーム(約0.267秒)に丸め。30fps下での最小丸め誤差、Low |
| **逸脱3: AAC微小信号** | 8.996〜8.999979秒に-65.0dBFSの微小信号(192サンプル)。#1〜#5・#10と同型のAAC前にじみ、Low(既存評価を踏襲) |
| 費用 | 実行前後とも119.37クレジットで差分0.00。Claudeが独立にJSON記録を確認(継続8回目) |

**Metricool予約投稿**: 実行済み。投稿ID`375103039`、2026-09-22 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。キャプション全文は2026-09-13にコーデックス経由でチャッピー原文(chappy-response-original.txt)を再取得し確認、要約から一言一句そのままの原文に差し替えた。
...
### #8 押し入れ 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式を受領。SHA-256整合性確認済み。絵コンテ目視確認済み。
プロジェクトID `closet08`。
...
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 13,056,993バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| NG③の視認性 | 探す時間から奥の容器の角が一部視認可能(解答時に追加なし)。設計どおり |
| **逸脱1: 解答円と背景の重なり** | NG①②③とも押し入れの枠・棚板・襖境界という**構造上の骨格線**への接触に限定。独立ダミー(布団・衣類等)との重なりなし。既存基準どおりLow |
| 逸脱2: 円描画の秒数 | 8フレーム(約0.267秒)への丸め。Low(#6と同型) |
| 逸脱3: AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS。Low(既存評価を踏襲) |
| 費用 | 実行前後とも119.37クレジットで差分0.00(継続9回目) |

**Metricool予約投稿**: 実行済み。投稿ID`375103072`、2026-09-24 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。キャプション全文はチャッピー原文をそのまま使用(改変なし)。
...
### #9 子どものおもちゃ 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式を受領。SHA-256整合性確認済み。絵コンテ目視確認済み。
プロジェクトID `toys09`。子ども本人は描画されていないことを絵コンテで確認。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 12,228,596バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| **逸脱1: 解答円と背景の重なり** | NG②は棚板前縁(支持面)でLow(既存基準)。**NG①は床のラグの輪郭に重なる**→4章新基準の3行目に該当。ラグを覆う面積は小さくおもちゃ本体は隠していないため**Low(条件付き承認)**。NG③は絵本棚(同一ダミーグループ)への接触でLow |
| 逸脱2: 円描画の秒数 | 8フレーム(約0.267秒)への丸め。Low |
| 逸脱3: AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS。Low(既存評価を踏襲) |
| 費用 | 実行前後とも119.37クレジットで差分0.00(継続10回目) |

**Metricool予約投稿**: 実行済み。投稿ID`375103077`、2026-09-25 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。キャプション全文はチャッピー原文をそのまま使用(改変なし)。
...
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 11,102,610バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| 解答円 | 使用しない構成のため該当なし |
| **逸脱: AAC微小信号** | 比較区間末尾5.996〜5.999979秒に-65.2dBFSの微小信号(192サンプル)。#6等と同型のAAC前にじみ、Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00(継続11回目) |
| 備考 | トピックバッジは9-Q確定構成の仕様どおり無し(該当なし) |

**Metricool予約投稿**: 実行済み。投稿ID`375091823`、2026-09-16 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。
...
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 12,872,548バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| 解答円 | 使用しない構成のため該当なし |
| **逸脱: AAC微小信号** | 比較区間末尾5.996〜5.999979秒に-65.2dBFSの微小信号(192サンプル)。クイズ#1と同型、Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00(継続12回目) |
| 備考 | トピックバッジは9-Q確定構成の仕様どおり無し(該当なし) |

**Metricool予約投稿**: 実行済み。投稿ID`375095181`、2026-09-21 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。
...
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 13,567,279バイト |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| NG②とNG③の識別 | ダミーの「立てたファイルボックス」との対比、マグカップをNG③の円外へ位置調整済み(コーデックスが自主的に修正) |
| **逸脱1: 解答円と背景の重なり** | NG①は家具・棚板等との重なりなし(クリーン)。NG③はデスク天板前縁(支持面)でLow(既存基準)。**NG②はデスク天板前縁に加え、椅子の背もたれ上端にも重なる**→4章新基準の3行目に該当。円が椅子を覆う面積は小さく書類本体は隠していないため**Low(条件付き承認)** |
| 逸脱2: 円描画の秒数 | 8フレーム(約0.267秒)への丸め。Low |
| 逸脱3: AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS。Low(既存評価を踏襲) |
| 費用 | 実行前後とも119.37クレジットで差分0.00(継続13回目) |

**Metricool予約投稿**: 実行済み。投稿ID`375103049`、2026-09-23 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。キャプション全文はチャッピー原文をそのまま使用(改変なし)。

---

### 9-11. #11 本棚

`【事実】` 2026-09-13、コーデックス経由でチャッピーから工程1〜3の回答を受領。企画書8-1章の提案
(「侵食」「無駄」等の評価語を除外)を反映済み。チャッピーから「NG②の言い換えはClaudeの確認対象」
との要確認事項があり、以下のとおりClaudeが確定した。

**Claudeの確定事項**: NG②の小物は「無地の直方体の小箱(本・書籍とは無関係、ブランド名・キャラクター
なし)」とする。実在の書名・出版社名・ロゴは使わない(チャッピーの要確認事項に対する回答)。

| 項目 | 内容 |
|---|---|
| トピックバッジ | `本棚` |
| シリーズラベル | `暮らしの間違い探し / 11` |
| 見出し | `ここ、` / `NGが3つあります`(固定・変更なしをコーデックスが照合済み) |

**一枚絵に描くもの**(本棚を正面から見た構図。複数段の棚)

| 要素 | 位置 | 種別 |
|---|---|---|
| 横向きに積まれた本(3冊) | 中段・中央 | **NG①(易)** |
| 本の列の隙間に置かれた無地の小箱(本と無関係、ブランド無し) | 下段・端 | **NG②(易)** |
| 隣り合う本2冊の上端の高さが不揃い | 上段・端(小さめ) | **NG③(難)** |
| 本棚の枠・棚板 | 全体の骨格 | ダミー(正常・空間) |
| 高さの揃った本の列(NG③との対比) | 別の段 | ダミー(正常) |
| ブックエンドで区切られた本のまとまり | 中段・別位置 | ダミー(正常) |
| 小さな観葉植物 | 棚の一角 | ダミー(正常) |
| 写真立て(本とは無関係だが自然な設置物) | 棚の一角 | ダミー(正常) |
| 何も置かれていない空の区画 | 別の段 | ダミー(正常・空間) |

**解答テロップ(チャッピー確定文言をそのまま採用)**

| # | 時間 | テロップ |
|---|---|---|
| ① | 10.00-11.33 | `本棚の本、` / `横向きに積んでいませんか` |
| ② | 11.33-12.67 | `本の横に、` / `小物を置いていませんか` |
| ③ | 12.67-14.00 | `並んだ本の高さ、` / `不揃いになっていませんか` |

**CTA**: `いくつ分かった? コメントで`
**キャプション**: チャッピー確定文言(8ブロック、テーマ「本棚」)をそのまま採用
**事実主張**: 無(形式Aのみ)。禁止事項の自己チェック(チャッピー実施分)を確認し、追加で
「実在の書名・出版社名・ロゴ不使用」を明記済み

### #11 本棚 確定版プロンプト(コーデックスにそのまま渡す)

```
Follow #1's visual style/safe areas/timeline exactly(9-1章参照).
SCENE: a multi-shelf bookshelf, viewed from front.
- NG1(easy): 3 books lying flat/horizontally stacked, center shelf.
- NG2(easy): a plain, UNBRANDED rectangular small box tucked into a gap
  in a row of books — clearly not a book, no logo/text/character design.
- NG3(hard): two adjacent books on an upper shelf, edge, with visibly
  MISALIGNED top heights (small/easy to overlook).
- Dummies(normal): shelf frame, a neatly height-aligned row of books
  (contrast to NG3, different shelf), a book section flanked by
  bookends, a small potted plant, a photo frame(no real photo/brand),
  one empty shelf section.
Series label "暮らしの間違い探し / 11". Badge "本棚".
[10.00-11.33] circle NG1 + "本棚の本、"/"横向きに積んでいませんか"
[11.33-12.67] circle NG2 + "本の横に、"/"小物を置いていませんか"
[12.67-14.00] circle NG3 + "並んだ本の高さ、"/"不揃いになっていませんか"
HARD CONSTRAINTS same as #1: no real book titles/publisher names/logos,
no "space is wasted" framing, no future-behavior/frequency claims,
no answer-in-question.
```

### #11 本棚 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式(HANDOFF全文・ffprobe・絵コンテ・残高記録・SHA-256、
参照設計書のgit blobハッシュ`c7e07b49c90a06821009f60bbf5e987c3db31a37`が本設計書コミット
`23781c0`のファイルと一致)を受領。整合性検証・絵コンテ目視確認済み。プロジェクトID `books11`。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 11,287,508バイト |
| **解答円と背景の重なり** | NG①②③とも棚板・枠という支持面、またはNG②の場合は「本の列の隙間」という同一文脈の本への接触に限定。独立した別ダミーとの重なりなし。既存基準どおりLow |
| AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS(既存6本と同一測定値)。Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00 |

**Metricool予約投稿**: 実行済み。投稿ID`375093097`、2026-09-17 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。

---

### 9-12. #12 冷凍庫

`【事実】` 2026-09-13、コーデックス経由でチャッピーから工程1〜3の回答を受領。企画書8-1章の
提案(将来行動・使用実態の断定を排除)を反映済み。チャッピーから「袋・容器の使い分け、表示の
有無、NG②の骨子修正はClaudeの確認対象」との要確認事項があり、以下のとおりClaudeが確定した。
食品領域につき、省エネ・電気代・食品衛生・賞味期限・鮮度への言及が無いことを再確認済み。

**Claudeの確定事項**:
1. NG①③を区別するため、NG①は「不透明な袋」(容器ではなく袋。中身の形も見せない単色無地)に限定する。
2. ラベルは**文字を描かない**。「白い無地の長方形シール」が貼ってあるかどうかの**有無のみ**で
   ラベルの有無を判別させる(具体的な文字・商品名・賞味期限表示は一切描かない)。
3. NG②は「ラベル付き透明容器2個が重なり、下の容器の中身が一部隠れている」状態とし、
   NG③(ラベル無し単独の透明容器)と混同しないよう、重なりの有無で区別する。

| 項目 | 内容 |
|---|---|
| トピックバッジ | `冷凍庫` |
| シリーズラベル | `暮らしの間違い探し / 12` |
| 見出し | `ここ、` / `NGが3つあります`(固定・変更なしをコーデックスが照合済み) |

**一枚絵に描くもの**(冷凍庫の中を正面から見た構図。棚板で仕切られた構図)

| 要素 | 位置 | 種別 |
|---|---|---|
| 中身が見えない不透明な袋(単色無地、ラベルなし) | 上段・中央 | **NG①(易)** |
| ラベル(白い無地シール)付き透明容器2個が重なり、下の容器の一部が隠れる | 中段 | **NG②(易)** |
| 他と接触しない透明容器、中身は見えるがラベル無し | 下段・端(小さめ) | **NG③(難)** |
| 冷凍庫内部の枠・仕切り棚 | 全体の骨格 | ダミー(正常・空間) |
| ラベル付き透明容器が整然と並ぶ列(対比) | 別の段 | ダミー(正常) |
| 保冷剤(単純な直方体) | 端 | ダミー(正常) |
| 製氷トレイ | 別位置 | ダミー(正常) |
| 仕切りケースの取っ手 | 端 | ダミー(正常・空間) |

**解答テロップ(チャッピー確定文言をそのまま採用)**

| # | 時間 | テロップ |
|---|---|---|
| ① | 10.00-11.33 | `この袋、` / `中身が隠れていませんか` |
| ② | 11.33-12.67 | `下の容器に、` / `別の容器が重なっていませんか` |
| ③ | 12.67-14.00 | `この容器、` / `ラベルなしで置いていませんか` |

**CTA**: `いくつ分かった? コメントで`
**キャプション**: チャッピー確定文言(8ブロック、テーマ「冷凍庫」)をそのまま採用
**事実主張**: 無(形式Aのみ)。省エネ・電気代・食品衛生・賞味期限・鮮度への言及なしを再確認

### #12 冷凍庫 確定版プロンプト(コーデックスにそのまま渡す)

```
Follow #1's visual style/safe areas/timeline exactly(9-1章参照).
SCENE: inside a freezer with shelf dividers, viewed from front.
- NG1(easy): an OPAQUE storage bag(plain, single color, no visible
  contents, no label/sticker), upper shelf, center.
- NG2(easy): two clear/transparent LABELED containers(plain white
  blank sticker = label, no text) stacked, the bottom one's contents
  partially hidden by the one on top.
- NG3(hard): a single clear/transparent container, NOT touching any
  other item, with NO label/sticker(contrast to NG2's labeled
  containers) — small, positioned lower shelf/edge.
- Dummies(normal): freezer frame/shelf dividers, a neat row of labeled
  (blank white sticker) clear containers on another shelf, an ice pack
  (simple block shape), an ice cube tray, a divider-case handle.
Series label "暮らしの間違い探し / 12". Badge "冷凍庫".
[10.00-11.33] circle NG1 + "この袋、"/"中身が隠れていませんか"
[11.33-12.67] circle NG2 + "下の容器に、"/"別の容器が重なっていませんか"
[12.67-14.00] circle NG3 + "この容器、"/"ラベルなしで置いていませんか"
HARD CONSTRAINTS same as #1: never mention energy cost, electricity,
food safety, hygiene, expiration/freshness dates. Labels are BLANK
white stickers only — never write readable text, brand names, or
dates on any label. No future-behavior/frequency claims, no
answer-in-question.
```

### #12 冷凍庫 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式を受領。参照設計書のgit blobハッシュが本設計書コミット
`23781c0`のファイルと一致。整合性検証・絵コンテ目視確認済み。プロジェクトID `freezer12`。
ラベルは白い無地シールのみで、文字・数字・商品名・日付は一切描かれていないことを確認。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 12,729,354バイト |
| **解答円と背景の重なり** | NG①②③とも冷凍庫の棚板・枠という支持面への接触に限定。独立した別ダミーとの重なりなし。既存基準どおりLow |
| AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS(既存と同一測定値)。Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00 |

**Metricool予約投稿**: 実行済み。投稿ID`375093103`、2026-09-18 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。
...
`【事実】` 2026-09-13、コーデックス経由でチャッピーから工程1〜3の回答を受領。企画書8-1章の
提案(使用実態・放置期間の断定を排除)を反映済み。NG③は「屋外用・屋内用」という用途分類を
チャッピー自身が骨子変更として削除し、形状差(ほうき/モップ)のみで判別する設計に修正済み。
チャッピーからの要確認事項に追加のClaude判断は不要と判断(標準解釈の適用範囲内)。

| 項目 | 内容 |
|---|---|
| トピックバッジ | `ベランダ` |
| シリーズラベル | `暮らしの間違い探し / 13` |
| 見出し | `ここ、` / `NGが3つあります`(固定・変更なしをコーデックスが照合済み) |

**一枚絵に描くもの**(ベランダを正面から見た構図。手すり・床を描く)

| 要素 | 位置 | 種別 |
|---|---|---|
| 床に置かれた植木鉢(1つ、枯れ・破損は描かない) | 手前・中央 | **NG①(易)** |
| 物干し竿に掛かったハンガー(複数、衣類なし) | 上部 | **NG②(易)** |
| 形状の異なる掃除道具の混在(ほうき+平たいヘッドのモップ) | 端(小さめ) | **NG③(難)** |
| ベランダの手すり・床の枠 | 全体の骨格 | ダミー(正常・空間) |
| 物干し竿本体(NG②との対比、竿自体は正常) | 上部・別位置 | ダミー(正常) |
| 排水口カバー | 床の隅 | ダミー(正常) |
| 室外機 | 端 | ダミー(正常) |
| きちんと束ねられたホース | 別位置 | ダミー(正常) |

**解答テロップ(チャッピー確定文言をそのまま採用)**

| # | 時間 | テロップ |
|---|---|---|
| ① | 10.00-11.33 | `ベランダの床に、` / `植木鉢を置いていませんか` |
| ② | 11.33-12.67 | `物干し竿に、` / `ハンガーが掛かっていませんか` |
| ③ | 12.67-14.00 | `掃除道具の種類、` / `混ざっていませんか` |

**CTA**: `いくつ分かった? コメントで`
**キャプション**: チャッピー確定文言(8ブロック、テーマ「ベランダ」)をそのまま採用
**事実主張**: 無(形式Aのみ)。道具の使用場所の断定なしを再確認

### #13 ベランダ 確定版プロンプト(コーデックスにそのまま渡す)

```
Follow #1's visual style/safe areas/timeline exactly(9-1章参照).
SCENE: a balcony, railing and floor, viewed from front.
- NG1(easy): a potted plant on the floor(no wilting/damage implied).
- NG2(easy): hangers left on the laundry pole(no clothes on them).
- NG3(hard): cleaning tools of DIFFERENT SHAPES mixed together(a broom
  + a flat-head mop) — small, positioned at one edge. Differentiate
  ONLY by shape, never by color-coding or "indoor/outdoor" labels.
- Dummies(normal): balcony railing/floor frame, the laundry pole itself
  (contrast to NG2), a drain cover, an outdoor AC unit, a neatly coiled
  hose.
Series label "暮らしの間違い探し / 13". Badge "ベランダ".
[10.00-11.33] circle NG1 + "ベランダの床に、"/"植木鉢を置いていませんか"
[11.33-12.67] circle NG2 + "物干し竿に、"/"ハンガーが掛かっていませんか"
[12.67-14.00] circle NG3 + "掃除道具の種類、"/"混ざっていませんか"
HARD CONSTRAINTS same as #1: never imply usage location(indoor/outdoor)
or usage frequency, no wilting/damage on the plant, no future-behavior
claims, no answer-in-question.
```

### #13 ベランダ 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式を受領。参照設計書のgit blobハッシュが本設計書コミット
`23781c0`のファイルと一致。整合性検証・絵コンテ目視確認済み。プロジェクトID `balcony13`。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 13,131,864バイト |
| **解答円と背景の重なり** | NG①③は床枠・手すり(単一の骨格ダミー)、NG②は物干し竿自体(対象物の支持体)への接触に限定。独立した別ダミーとの重なりなし。既存基準どおりLow |
| AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS(既存と同一測定値)。Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00 |

**Metricool予約投稿**: 実行済み。投稿ID`375093155`、2026-09-19 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。
...
`【事実】` 2026-09-13、コーデックス経由でチャッピーから工程1〜3の回答を受領。企画書8-1章の
提案(開封履歴・滞留期間の断定を排除)を反映済み。チャッピーから「『保留』表示の可読性と
ヒント表示禁止との整合をClaudeが確認」との要確認事項があり、以下のとおりClaudeが確定した。

**Claudeの確定事項**: 「保留」の2文字は、箱そのものの名称・識別表示であり、探す時間(2.00-9.00秒)
から答え合わせ後まで**一貫して同じ状態で表示**する(解答時に新規追加・強調・色変更をしない)。
6章「探す時間中にヒントやチェックマークを出さない」は、**解答位置を示す一時的な合図**を禁止する
規定であり、対象物自体の恒常的な名称表示(「保留」の箱名)はこれに該当しないと判断する。

| 項目 | 内容 |
|---|---|
| トピックバッジ | `書類・郵便物` |
| シリーズラベル | `暮らしの間違い探し / 14` |
| 見出し | `ここ、` / `NGが3つあります`(固定・変更なしをコーデックスが照合済み) |

**一枚絵に描くもの**(テーブル上を正面から見た構図)

| 要素 | 位置 | 種別 |
|---|---|---|
| テーブルに置かれた、封が閉じた封筒(裏面からフラップの閉状態が分かる) | テーブル・手前中央 | **NG①(易)** |
| 大きさ・形の異なる紙が透けて見えるクリアファイル | テーブル・脇 | **NG②(易)** |
| 「保留」と表示された箱に積まれた書類(層が見える) | 上部または端(小さめ) | **NG③(難)** |
| テーブル本体 | 全体の骨格 | ダミー(正常・空間) |
| 開封済みで整理された封筒の束(対比) | 別位置 | ダミー(正常) |
| 大きさの揃った紙が入ったクリアファイル(対比) | 別位置 | ダミー(正常) |
| ペン立て | テーブル端 | ダミー(正常) |
| カレンダー | 背景 | ダミー(正常) |
| ラベルの無い、中身が整理された別の箱(対比) | 別位置 | ダミー(正常) |

**解答テロップ(チャッピー確定文言をそのまま採用)**

| # | 時間 | テロップ |
|---|---|---|
| ① | 10.00-11.33 | `テーブルの封筒、` / `封が閉じていませんか` |
| ② | 11.33-12.67 | `このクリアファイル、` / `違う形の紙が入っていませんか` |
| ③ | 12.67-14.00 | `「保留」の箱に、` / `書類が積まれていませんか` |

**CTA**: `いくつ分かった? コメントで`
**キャプション**: チャッピー確定文言(8ブロック、テーマ「書類・郵便物」)をそのまま採用
**事実主張**: 無(形式Aのみ)。開封履歴・滞留期間・処理状況の断定なしを再確認

### #14 書類・郵便物 確定版プロンプト(コーデックスにそのまま渡す)

```
Follow #1's visual style/safe areas/timeline exactly(9-1章参照).
SCENE: a tabletop with envelopes/documents, viewed from front.
- NG1(easy): an envelope on the table shown from the BACK, flap
  visibly SEALED/closed — a front-only view does not satisfy this NG.
- NG2(easy): a clear plastic file sleeve with papers of DIFFERENT
  sizes/shapes visible through it(silhouettes only, no readable text).
- NG3(hard): a box labeled "保留"(2 characters only, no other text)
  with a visible stack/layer of documents inside — the "保留" label
  and the paper stack must be visible identically from 2.00s onward;
  do NOT add/emphasize/recolor anything at reveal time.
- Dummies(normal): table body, a tidy bundle of opened envelopes
  (contrast to NG1), a file sleeve with uniformly-sized papers
  (contrast to NG2), a pen holder, a calendar(background), a second
  unlabeled box with tidy contents(contrast to NG3).
Series label "暮らしの間違い探し / 14". Badge "書類・郵便物".
[10.00-11.33] circle NG1 + "テーブルの封筒、"/"封が閉じていませんか"
[11.33-12.67] circle NG2 + "このクリアファイル、"/"違う形の紙が入っていませんか"
[12.67-14.00] circle NG3 + "「保留」の箱に、"/"書類が積まれていませんか"
HARD CONSTRAINTS same as #1: never assert unopened history, unprocessed
status, or how long items sat there — describe only visible placement.
No arrows/highlight color added at reveal. No answer-in-question.
```

### #14 書類・郵便物 納品記録(2026-09-13)

`【事実】` コーデックスから一次データ一式を受領。参照設計書のgit blobハッシュが本設計書コミット
`23781c0`のファイルと一致。整合性検証・絵コンテ目視確認済み。プロジェクトID `papers14`。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p |
| ファイルサイズ | 12,482,006バイト |
| **解答円と背景の重なり** | NG①③はテーブル前縁・奥辺(支持面)への接触に限定。**NG②は他のNG・独立ダミーとの重なりなし(クリーン)**。既存基準どおりLow |
| **「保留」表示の固定性** | 元PNG(検索・解答①②③・CTA相当の5点)では同一(RGB差0)を確認。**納品MP4では351フレーム中6種類の画素パターン、最大RGB差10が測定された**。これはH.264圧縮由来の非可逆ノイズであり、文字・位置・サイズ・色の意図的変更ではないことをlabel-verification.jsonで確認。AAC微小信号と同種の「エンコード起因の技術的ノイズ」としてLow評価 |
| AAC微小信号 | 8.996〜8.999979秒、-65.0dBFS(既存と同一測定値)。Low |
| 費用 | 実行前後とも119.37クレジットで差分0.00 |

**Metricool予約投稿**: 実行済み。投稿ID`375093163`、2026-09-20 20:00(Asia/Tokyo)公開予定、`autoPublish=true`、`isAiGenerated=false`。
...
| 2026-09-13 | **#6・#7・#8・#9・クイズ#1・#2の動画納品を確認**。コーデックスから一次データ一式(HANDOFF全文・ffprobe生出力・絵コンテ・音声詳細・残高記録・SHA-256)を受領し、Claudeが同梱ファイルの整合性を独立検証(全件一致)。絵コンテを目視確認。4章に「解答円と背景要素の重なりに関する評価基準」を新設し、#6 NG②(ラグ)・#7 NG②(椅子)・#9 NG①(ラグ)という、支持面ではなく独立ダミーの輪郭に円が部分的に重なる新パターンをLow(条件付き承認)と判定。AAC微小信号(-65dBFS前後、192サンプル)は全6本で既存評価(Low)を踏襲。6本とも「次工程: 関野さんの実機確認待ち」に移行 |
| 2026-09-13 | **#11本棚・#12冷凍庫・#13ベランダ・#14書類郵便物の設計を確定**。コーデックス経由でチャッピーの工程1〜3回答(原文)を受領。9-11〜9-14章を新設。チャッピーの要確認事項に対しClaudeが以下を決定: #11 NG②の小物を「無地の小箱(ブランド・書名なし)」に確定、#12はラベルを「白い無地シールの有無のみ」で表現し文字を描かないことに確定、#14の「保留」表示は探す時間から答え合わせまで一貫表示する恒常的な名称表示であり6章のヒント禁止規定には抵触しないと判断。#13は骨子変更(用途分類の削除)をそのまま承認。4本とも確定版プロンプトを作成し、次工程(コーデックスによる動画生成)へ引き継ぎ |
| 2026-09-13 | **#11〜#14の動画納品を確認**。コーデックスから一次データ一式を受領し、参照設計書のgit blobハッシュが直前のコミット(`23781c0`)のファイルと完全一致することを確認(設計ドリフトなし)。4本とも解答円の重なりは既存の「支持面」基準内に収まり、#6〜#9で新設した「独立ダミーとの重なり」例外の適用は不要だった。#14では「保留」表示がH.264圧縮由来の微小ノイズ(最大RGB差10、元画像は完全一致)を示すのみで、意図的な変更でないことを確認。4本とも設計書に納品記録を追記し、関野さんの実機確認待ちに移行 |
| 2026-09-13 | **クイズ#1・クイズ#2・#11・#12・#13・#14をMetricool予約投稿(関野さんの実機確認・承認を受けて実行)**。投稿ID: クイズ#1靴下=375091823(9/16)、#11本棚=375093097(9/17)、#12冷凍庫=375093103(9/18)、#13ベランダ=375093155(9/19)、#14書類郵便物=375093163(9/20)、クイズ#2ネックレス=375095181(9/21)。いずれも20:00(Asia/Tokyo)・autoPublish=true・isAiGenerated=false、キャプションは設計書に確定済みの全文をそのまま使用(改変なし)。**#6・#7・#8・#9は、設計書に「キャプション8ブロック確定」という要約のみが記録されており、チャッピー原文の全文が本セッションのデータに残っていなかったため、予約投稿を保留**(要約から再構成したキャプション案を関野さんに確認依頼中)。#5洗面所・#10食品ストックも、動画URLを保持していないため今回の予約投稿には含めていない |
| 2026-09-13 | **#6・#7・#8・#9のキャプション原文、#5・#10の動画URLをコーデックスから再取得し、6本すべてを予約投稿**。チャッピー原文(chappy-response-original.txt)を照合した結果、Claudeが要約から再構成した案とは細部の言い回しが異なっていた(例: #6 NG③は「3箇所に分かれて置かれたリモコン」ではなく「別々の場所に置かれたリモコン」)ため、推測での投稿を避けたことが正しかったと確認できた。投稿ID: #6リビング=375103039(9/22)、#7デスク=375103049(9/23)、#8押し入れ=375103072(9/24)、#9子どものおもちゃ=375103077(9/25)、#5洗面所=375106106(9/26)、#10食品ストック=375107608(9/27)。いずれもチャッピー原文のキャプションをそのまま使用。**これで#1〜#14・クイズ#1・#2の全16本がMetricool予約投稿まで完了した** |
