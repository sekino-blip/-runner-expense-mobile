...
### #10 食品ストック 納品記録(2026-09-12)

`【事実】` コーデックスから納品。プロジェクトID `pantry10`。

| 項目 | 結果 |
|---|---|
| 総尺/解像度/fps/フレーム数 | 15.000秒(差分0.000秒) / 1080×1920 / 30fps / 450フレーム |
| エンコード | H.264 High / yuv420p(標準プロファイル) |
| ファイルサイズ | 13,385,975バイト(約12.8MB) |
| 音声 | AAC LC 48kHz mono。音声尺15.000000秒(映像と完全一致) |
| 設計書11章チェックリスト | 17項目すべて合格 |
| **NG②の楕円ハイライト** | 承認済み例外どおり実装(幅160px・高さ292px、テラコッタ線幅6px・塗りなし)。NG①③は標準の160px円を維持 ✓ |
| NG③の視認性 | 探索区間210フレーム全てで奥の容器の右端が同一ピクセル。解答時の追加なし |
| **逸脱事項(要評価)** | ハイライトの一部が支持面(床の接地線・棚板・戸棚外枠)に重なる。**Low評価**: #1・#5と同種の許容パターン。接地・2段の関係を示す構図上、自然かつ必然。他のNGや独立小物とは重ならない |
| その他の逸脱 | AAC由来の音の前にじみ(9秒直前、約-65dBFS)。継続的なLow評価 |
| 費用 | 実行前後とも119.37クレジットで差分0.00。**Claudeが独立に`balance`/`transactions`を再確認し、新規取引が無いことも確認済み**(7回連続で同じ結果) |
...
`【事実】` 2026-09-12、コーデックス経由でチャッピーから納品。企画書8-1章の提案(「大量のペン」)を
さらに具体化し、「密に詰まっている」という視覚的に確認可能な状態に落とし込んだ。
「実際に使うのは2本」「取り出す時間が減る」「何本以上は多すぎる」をすべて不採用として除外。

| 項目 | 内容 |
|---|---|
| トピックバッジ | `デスク` |
| シリーズラベル | `暮らしの間違い探し / 07` |
| 見出し | `ここ、` / `NGが3つあります`(固定) |

**一枚絵に描くもの**(デスク周りを正面から見た構図。天板と床の一部を描く)

| 要素 | 位置 | 種別 |
|---|---|---|
| 床で複数のケーブルが絡み合っている(交差だけでなく絡みと分かる描写) | **デスク下・床** | **NG①(易)** |
| デスク上に書類が横積みで重なっている(寝かせて重ねる。立てたファイルとは区別) | **デスク上・中央** | **NG②(易)** |
| ペン立ての口にペンが隙間なく密集(**隙間が十分あると不成立。密着を明確に描写**) | **上部または端・小さめ** | **NG③(難)** |
| デスク本体・天板 | 全体の骨格 | ダミー(正常・空間) |
| 椅子 | 背景 | ダミー(正常・空間) |
| 立てて整理されたファイルボックス(NG②との対比) | デスク上・端 | ダミー(正常) |
| ノートパソコン、閉じて置かれている | デスク上・端 | ダミー(正常) |
| デスクライト | デスク上・奥 | ダミー(正常) |
| マグカップ、コースターの上にきちんと置かれている | デスク上・別位置 | ダミー(正常) |

**チャッピーの4つの描画確認事項への対応**

| # | 確認事項 | Claudeの対応 |
|---|---|---|
| 1 | NG①は交差だけでなく絡んだ状態と識別できること | 複数のケーブルが結び目のように重なり合う描写にする |
| 2 | NG②は寝かせて重ねた描画、立てたファイル群と区別 | ダミーに「立てて整理されたファイルボックス」を配置し対比させる |
| 3 | NG③は口にペンが密に並ぶ状態が条件。隙間があると不成立 | ペン同士の間隔をゼロに近く描画し、容器の縁まで埋まっている状態にする |
| 4 | 「使うのは2本」等は不採用 | 解答テロップに反映済み(下記) |

**解答テロップ(C4)** ← **確定(チャッピー納品版。形式Aのみで構成、出典不要)**

| # | 時間 | テロップ |
|---|---|---|
| ① | 10.00-11.33 | `床のケーブル、` / `絡まっていませんか` |
| ② | 11.33-12.67 | `デスクの書類、` / `積み重なっていませんか` |
| ③ | 12.67-14.00 | `ペン立てに、` / `ペンが詰まっていませんか` |

**CTA**: 動画`いくつ分かった? コメントで` / キャプション`いくつ分かった?コメントで`

**キャプション**: 8ブロック確定(テーマ「デスク」、最初の一歩「まずはペン立ての中を、見直すところから。」、ハッシュタグ5個)

**事実主張**: 無(使用実態・効果・本数基準はすべて不採用として除外済み)

### #7 デスク 確定版プロンプト(コーデックスにそのまま渡す)

```
Follow #1's visual style/safe areas/timeline exactly(9-1章参照).
SCENE: a desk area with chair and floor, viewed from front.
- NG1(easy): multiple cables on the floor under the desk, visibly TANGLED
  together(not just crossing — a knot-like overlap).
- NG2(easy): documents stacked FLAT/horizontally on the desk, center —
  contrast with a dummy upright file box elsewhere.
- NG3(hard): a pen holder with pens packed so densely there is NO visible
  gap between them or to the rim — small, positioned upper/edge. If gaps
  are visible between pens, it does not read as "packed."
- Dummies(normal): desk body/top, a chair(background), an upright file
  box(contrast to NG2), a closed laptop, a desk lamp, a mug on a coaster.
Series label "暮らしの間違い探し / 07". Badge "デスク".
[10.00-11.33] circle NG1 + "床のケーブル、"/"絡まっていませんか"
[11.33-12.67] circle NG2 + "デスクの書類、"/"積み重なっていませんか"
[12.67-14.00] circle NG3 + "ペン立てに、"/"ペンが詰まっていませんか"
HARD CONSTRAINTS same as #1: never state pen count as a threshold, never
assert actual usage("使うのは2本"等)or efficiency effects.
```
...
| 2026-09-12 | **#7デスクを確定**。コーデックス経由でチャッピーから納品を受け、禁止事項チェック合格。8-1章の提案(「大量のペン」)をさらに具体化し「密に詰まっている」という視覚状態に落とし込んだ。一枚絵の配置を9-7章に確定 |
