...
(企画書9-2章にmp4 URL記載)からffmpegでフレームを抽出し、背景色・見出し(深緑)・
バッジ(テラコッタ)のHEX値をピクセルサンプリングして報告する。
...
**色**(`【推測】` **0-6-1により未確定。コーデックス側での抽出結果が届き次第、確定値に置き換える**)

| 要素 | プレースホルダー | 確定方法 |
|---|---|---|
| 背景 | クリーム `#F0E9DC`(暫定) | 既存投稿(9/8 or 9/9)からffmpegでフレーム抽出しピクセルサンプリング |
| 見出し(深緑) | 未確定 | 同上 |
| バッジ(テラコッタ) | 未確定 | 同上 |
| 解答マーカーの円 | バッジと同色、線のみ・塗りつぶし無し | 同上 |
