...
> このファイルはClaude / ChatGPT(Codex)双方に同じ文脈を共有するための共通ドキュメントです。
...
`【事実】` 2026-09-11時点で確認した分担。

| 担当 | 範囲 |
|---|---|
| **ChatGPT / Codex** | **動画生成(HyperFrames / HeyGen)**。本設計書を入力としてcomposeを実行 |
...
したがって動画生成は必ずCodex側で実行する。

`【事実】` Claudeの実行環境では `files2.heygen.ai` および `static.metricool.com` への
直接アクセスが組織プロキシに拒否される(CONNECT 403)。**Claudeは完成動画を目視確認できない。**
→ **納品時は、Codex側またはDropbox経由で内容が確認できる形(絵コンテ画像・全テロップの文字起こし)を添えること。**
中身を確認できない動画を予約投稿することはできない。
